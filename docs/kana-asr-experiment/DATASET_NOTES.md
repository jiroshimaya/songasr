...
## 1b. 疑似ラベルデータセットのスケールアップ(v2, 1000文)

ユーザーとの合意で、#2(正解データ作成)は当面「TTS疑似ラベルの規模拡大」を主軸にする方針になった(実歌唱の追加収集はPixabayがCloudflareでブロックされ automatable ではないため、27区間の1曲データは検証用の少数ホールドアウトとして維持し、深追いしない)。

- **文の多様性**: v1(120文)は主語+動詞句+タグという単一テンプレートが中心だったため、v2 (`scripts/kana_dataset_sentences_v2.py`) では (a) 子音クラスタ(str/spl/scr/thr/spr等)・二重母音(aɪ/aʊ/ɔɪ/eɪ/oʊ)・母音の多い言い回し・縮約形・短文(2〜3語)〜長文(12〜14語)を狙って書いた手書きシード文 約100文と、(b) 平叙文・命令文・疑問文・倒置・複文など**5種類の異なる文構造テンプレート**を均等にローテーションさせた生成文を組み合わせている。単一パターンの反復にならないよう構造自体を複数用意したのがv1からの主な改善点。
- **アクセントの多様性**: gTTSは話者を選べないため、`tld`パラメータ(`com`/`co.uk`/`com.au`/`co.in`/`ca`)を1000文に均等ローテーション(各200文)して割り当てた。同一エンジンではあるが地域アクセントの発音差が入るため、単一ボイスへの過学習を多少なりとも緩和できることを期待している。
- **規模**: 1000文(`scripts/build_kana_dataset_v2.py --n 1000`)。CPU上で約15分(1文あたり約0.85秒、gTTS読み上げ+baselineの疑似ラベル付与込み)で完了した。
- **品質チェック**: 空ラベル 0/1000。手書きシード文(子音クラスタ・二重母音など難しいパターンを狙ったもの)とテンプレート生成文の両方から無作為抽出して目視した結果、"Strength"→`ストゥレンス`、"Splash"→`スプラッシャ`、"Scratch"→`スクラッチ`のように、狙った難しい音韻パターンでも認識自体は破綻しておらず(空文字・意味不明な繰り返しなどのガラベージ出力は見られない)、baselineパイプラインが疑似ラベラーとして機能していることを確認した。もちろんこれは「ガラベージでない」ことの確認であって、「正しい」ことの保証ではない(#1の弱教師ありという限界は変わらない)。
- **出力**: 音声は `local/kana_dataset_v2/`(gitignore対象、1000件、数百MB規模)。マニフェスト全件は [`data/kana_dataset_v2/manifest.jsonl`](../../data/kana_dataset_v2/manifest.jsonl) にコミットしている(音声そのものはコミットせず、`scripts/build_kana_dataset_v2.py`を再実行すれば同じ文・同じtld順序で再生成可能)。マニフェストの各行は `{id, text, tld, audio_file, duration_sec, kana}`。
- **現時点の位置づけ**: この1000文のTTS疑似ラベルセットが、#3(モデル学習)の**主たる学習データ候補**。前回作った27区間の実歌唱データ(1曲分、`local/song_corpus/results.json`)は規模・多様性(1曲・1人の歌手)の観点で学習には使わず、**検証用の少数ホールドアウトセット**として維持する。
...
## 次にやるべき具体的なこと(#2の次の一歩の提案、ユーザー方針確認済み)

ユーザーとの方針確認の結果、**当面は実歌唱データの追加収集を追わず、TTS疑似ラベル(v2, 1000文)を主軸に#3(モデル学習)へ進む**ことになった。実歌唱の27区間は検証用ホールドアウトとして維持する。

1. **#3(モデル学習)は次のセッション/イシューで着手する(今回は未着手)。** 学習データは `data/kana_dataset_v2/manifest.jsonl`(1000文、音声は `scripts/build_kana_dataset_v2.py` で再生成)を主に使う想定。
2. 評価は `local/song_corpus/results.json`(実歌唱27区間)をホールドアウトとして使い、TTS疑似ラベルで学習したモデルが実歌唱にどこまで汎化するかを見る(#4 検証との接続点)。
3. 音声拡張は「一様なピッチシフト/テンポ変更」だけでなく、母音部分だけを狙って伸ばす・ピッチ変調するような、より歌唱に忠実な拡張を今後検討する。
4. ボーカル分離は「前処理として常に使うもの」ではなく、**選択肢の一つとして評価に含める**(分離あり/なしの両方でモデルを評価し、実際に効果があるか確認する)程度の位置づけに留める。
5. 商用利用可な実歌唱トラックの追加収集(Pixabayの手作業探索など)は、必要になった時点で改めてユーザー主導で行う。優先度は下げた。
